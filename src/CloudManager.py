...
def clean_up(data):
    ...
    # TODO: replace BUCKET and project --> get environ-variable
    BUCKET = 'clopper-storage'
    #my_env = os.environ.copy()
    #my_env['GOOGLE_CLOUD_PROJECT'] = "bt-sfabel"
    storage_client = storage.Client(project=data['gce-project'])
...
def start_grpc_server(node_dict):
    ...
    for node in node_dict.iteritems():
        ...
        ip = node[1]
        # TODO: check server-location
        cmd = "ssh -o StrictHostKeyChecking=no -L 222" + port + ":localhost:8080 $USER@" + ip + ' python ~/server.py'
...
def distribute_test_suite(node_dict, test_suite, data):
    ...
    for node, bundle in zip(node_dict.iteritems(), test_suite):
        ...
        stdin, stdout, stderr= client.exec_command('mkdir ~/tmp')
        with SCPClient(client.get_transport()) as scp:
            scp.put(config, expanduser('~/tmp/config.tar.gz'))
            scp.put(cl, expanduser('~/tmp/params.tar.gz'))
            scp.put(project, expanduser('~/tmp/project.tar.gz'))
...
def parse_json(param):
    ...
    paras = ('mode', 'total', 'CL-params', 'project', 'distribution')
    ...
    if not '--cloud' in data['CL-params'] or data['CL-params']['--cloud'] == False:
        data['CL-params']['--cloud'] = True
